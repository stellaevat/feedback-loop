...
class AssignmentForm(forms.ModelForm):
    name = forms.CharField(max_length=100, help_text=mark_safe('<b>Name: </b>'), widget=TextInput(attrs={'placeholder': 'Enter assignment name'}))
    spec = forms.FileField(required=False, help_text=mark_safe('<p>Specification: </p>'), widget=ClearableFileInput())
    rubric = forms.FileField(required=False, help_text=mark_safe('<p>Rubric: </p>'), widget=ClearableFileInput())
    markers = forms.ModelMultipleChoiceField(required=False, help_text=mark_safe('<b>Markers assigned: </b>'), queryset=User.objects.filter(userprofile__is_marker=True), widget=SelectMultiple(attrs={"size":'1'}))
...
class GradeModerationForm(forms.ModelForm):
    grade_moderation = forms.ChoiceField(help_text=mark_safe('<p>Grade moderation:</p>'), choices=[(None, '')] + list(Assignment.MODERATION_ALGORITHMS), widget=Select(attrs={'id': 'grade-moderation-alg',  'onchange': 'revealModerationParams(this)'}))
    grade_moderation_param_1 = forms.FloatField(help_text=mark_safe('<span id="param-1-text"></span>'), widget=NumberInput(attrs={'id': 'param-1', 'class':'short-input', 'step': '0.1', 'hidden': True}))
    grade_moderation_param_2 = forms.FloatField(help_text=mark_safe('<span id="param-2-text"></span>'), widget=NumberInput(attrs={'id': 'param-2', 'class':'short-input', 'step': '0.1', 'hidden': True}))
    ...
    def __init__(self, *args, **kwargs):
        ...
        if assignment is not None:
            if assignment.grade_moderation is not None:
                ...
                self.fields['grade_moderation_param_1'].widget = NumberInput(attrs={'id': 'param-1', 'class':'short-input', 'step': '0.1'})
                self.fields['grade_moderation_param_2'].widget = NumberInput(attrs={'id': 'param-2', 'class':'short-input', 'step': '0.1'})
...
class FeedbackModerationForm(forms.ModelForm):
    feedback_tone = forms.ChoiceField(required=False, help_text='Desired tone: ', choices=[(None, '')] + list(Assignment.TONES), widget=Select())
    feedback_length_min = forms.IntegerField(required=False, min_value=0, help_text='Min. feedback length: ', widget=NumberInput(attrs={'placeholder':'min', 'class':'short-input'}))
    feedback_length_max = forms.IntegerField(required=False, min_value=0, help_text='Max. feedback length: ', widget=NumberInput(attrs={'placeholder':'max', 'class':'short-input'}))
    feedback_moderation_requests = forms.CharField(required=False, help_text='Additional requests: ', widget=forms.Textarea(attrs={'rows': 6, 'placeholder': 'Describe any additional moderation requests...'}))
...
class MarkedSubmissionForm(forms.ModelForm):
    student_cid = forms.CharField(max_length=8, help_text=mark_safe('<b>Student CID: </b>'), widget=TextInput(attrs={'placeholder': 'Enter student CID'}))
    marker_grade = forms.FloatField(required=False, min_value=0, max_value=100, help_text=mark_safe('<b>Grade: </b>'), widget=NumberInput(attrs={'step': '0.1'}))
    initial_marker_feedback = forms.CharField(required=False, help_text=mark_safe('<p>Feedback: </p>'), widget=forms.Textarea(attrs={'rows': 6, 'placeholder': 'Enter your feedback...', 'class':'small-indent'}))
    suggested_improvements = forms.CharField(required=False, help_text=mark_safe('<p id="suggestion-label">Suggested improvements: </p>'), widget=forms.Textarea(attrs={'readonly': True, 'rows': 6, 'class':'small-indent'}))
    improved_marker_feedback = forms.CharField(required=False, help_text=mark_safe('<p>Final feedback: </p>'), widget=forms.Textarea(attrs={'disabled': True, 'rows': 6, 'placeholder': 'Enter your final improved feedback...', 'class':'small-indent'}))
    ...
    def __init__(self, *args, **kwargs):
        ...
        if submission is not None:
            ...
            if submission.suggested_improvements:
                self.fields['improved_marker_feedback'].widget = forms.Textarea(attrs={'rows': 6, 'placeholder': 'Enter your final improved feedback...', 'class':'small-indent'})
    ...
    def clean(self):
        ...
        same_submissions = MarkedSubmission.objects.filter(assignment=self.assignment, marker=self.marker, student_cid=student_cid)
        if self.submission:
            # Not considering the submission itself when being updated
            same_submissions = same_submissions.exclude(pk=self.submission.pk)
        
        if same_submissions.exists():
